...
統計的検定の思考フレームワーク
棄却を目的として，帰無仮説をこしらえる
帰無仮説のもと，標本からある分布（正規分布，t分布，F分布，2分布など）に従う統計量Tを求める
統計量Tがその分布のもとで稀なケースかどうかをチェックする
稀なケースであり，帰無仮説を疑うほうが妥当であると判断できる場合は帰無仮説を棄却し，対立仮説を採択する。そうでないなら何もわからないと判断する
3で「稀なケース」と判断する手段について，p値の役割と共に説明します。
...
Welch の片側検定
N(A,2A), N(B,2B)で，2A=2Bかも不明なら，Welchの片側検定と呼ばれる検定が使用できます。この場合，以下のようなt分布に従う統計量Tと自由度mを使ってp値を求め検定を行います。
帰無仮説 H0:A=B ,  対立仮説 H1:A>B
統計量 T=XA-XB/S2A / (nA-1)+S2B / (nB-1)
自由度 m=S2AnA-1+S2BnB-12/ S4AnA-13+S4BnB-13
ただし，AおよびBは平均，SAおよびSBは標準偏差
テストに必要なインプットテーブル形式
...
購買履歴での事例
具体的な例で見ていきましょう。以下は，（あまり比較の意味がないですが）sales_slipのmember_idの偶数と奇数でAとBに分け，2011年における平均購入額に差異があるのかを検定するクエリです。
SELECT 
...
先ほどは，
（サンプル数）A：4400人，B：4400人
（平均購入額）A：16.6万円，B：20.5万円
（標準偏差）A：220000，B：2450000
t_stat = 1.05 < 1.96 = t_val ⇒ 「NOT REJECT」
という結果でした。下図はこの状況を可視化したものです。

ここで，テストの知見を深めるために以下の考察を行いましょう。
A,B 双方のサンプル数がもっと多い/少ない場合，テストの結果はどう変わるか？
Aのサンプル数がBに対して圧倒的に少ない場合，テストの結果はどう変わるか？
A,B 双方の標準偏差がもっと大きい/小さい場合，テストの結果はどう変わるか？
Aの標準偏差がBに対して圧倒的に大きい場合，テストの結果はどう変わるか？
これらをその他の条件は変えずに検証（t_statとt_valueの値とその位置関係がどう変わるか？）したいと思います。
1-a. A,B 双方のサンプル数がもっと多い場合
まず，サンプル数がもっと多い場合を先に考えます。
（サンプル数）A：440000人，B：440000人 ←100倍にしてみる
（平均購入額）A：16.6万円，B：20.5万円
（標準偏差）A：220000，B：2450000
...
（サンプル数）A：44人，B：4400人 ←Aを1/100にしてみる
（平均購入額）A：16.6万円，B：20.5万円
（標準偏差）A：220000，B：2450000
WITH ab_table AS
(
...
（サンプル数）A：4人，B：440000人 ←Aを1/1000，Bを100倍
（平均購入額）A：16.6万円，B：20.5万円
（標準偏差）A：220000，B：2450000
...
（サンプル数）A：4400人，B：4400人
（平均購入額）A：16.6万円，B：20.5万円
（標準偏差）A：22000000，B：245000000 ←100倍にしてみる
WITH ab_table AS
(
  SELECT 
    IF(CAST(member_id AS INTEGER)%2=0,'A','B') AS ab, 
...
（サンプル数）A：4400人，B：4400人
（平均購入額）A：16.6万円，B：20.5万円
（標準偏差）A：2200，B：24500 ←1/100にしてみる
WITH ab_table AS
(
  SELECT 
    IF(CAST(member_id AS INTEGER)%2=0,'A','B') AS ab, 
...
（サンプル数）A：4400人，B：4400人
（平均購入額）A：16.6万円，B：20.5万円
（標準偏差）A：22000000，B：24500 ←Aを100倍，Bを1/100にしてみる
WITH ab_table AS
(
...
今回のテストで差異が認められたのは3県だけでしたが，これはそもそもA/Bの分類が適当だったことに起因します。また，この判定は単純な平均値の近さだけでなく，各々のレコード数と分散を考慮した結果になっていることに注意してください。
アクセスログでの事例
